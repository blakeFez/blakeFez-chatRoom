...
class WebSocket{
	...
	function getAndSend($data, $accept, $index){
		...
		if(!empty($data['time'])){
			$time = time();
			$this->time[$index] = $time;
			return $this->send(array('time'=>$time), $index);
			...
			if(!empty($this->bind[$index]['name'])){
				return  $this->send(array('msg'=>'<div class="msg error">你已经有名称了</div>'), $index);
			}
			$list = array();
			//判断名字是否已经存在
			...
			return $this->send(array('name' => true, 'msg' => '<div class="msg yes">你已经成功登录上聊天室</div>'), $index);
		}elseif(!empty($data['chat'])){//聊天内容
			$name  = empty($this->bind[$index]['name']) ? '' : $this->bind[$index]['name'];
			$chat = (string)$data['chat'];
			if(!$name){
				return $this->send(array('msg' => '<div class="msg error">你还没有输入你的名称</div>'), $index);
			}
		
			return $this->sendAll(array('chat'=>'<div class="chat"><div class="name">'.$name.'</div><p>'.$chat.'</p></div>'));
		}
	}
